        ...
        for row in buttons:
            for index, button in enumerate(row):
                if index % 4 == 0 and index != 0:
                    keyboard.add_line()
                keyboard.add_button(button['action']['label'], color=VkKeyboardColor.PRIMARY)
...
def add_beer_stage(user_id, message, attachments=None):
    ...
    if state is None:
        ...
        # Сохраняем данные о пиве и переходим на стадию ожидания фото
        name, category, subcategory, volume, alcohol, country, price, description, photo_url = args
        admin_states[user_id] = {
            "name": name, "category": category, "subcategory": subcategory, "volume": volume,
            "alcohol": alcohol, "country": country, "price": price, "description": description, "photo_url": photo_url,
            "stage": "awaiting_photo"
        }
        
        # Сообщение об успешном добавлении пива с фото
        send_message(user_id, "🍺 Пиво успешно добавлено с фото! 🖼")

        # Сразу сохраняем пиво в базу данных, так как фото — это URL
        try:
            with db.cursor() as cursor:
                sql = """INSERT INTO beers (name, category, type, volume, alcohol, country, price, description, photo_url)
                         VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)"""
                cursor.execute(sql, (
                    name, category, subcategory, volume, float(alcohol), country, float(price), description, photo_url
                ))
                db.commit()

            # Сообщение о успешном добавлении в базу данных
            send_message(
                user_id, 
...
def delete_beer_by_id(user_id, beer_id):
    try:
        ...
        send_message(
            user_id, 
            f"🗑️ Пиво с ID {beer_id} успешно удалено!\n"
        ...
        send_message(
            user_id, 
            f"❌ Ошибка при удалении пива с ID {beer_id}: {e}\n"
...
def view_all_beers(user_id):
    with db.cursor() as cursor:
        cursor.execute("SELECT id, name, category, type, price, photo_url FROM beers")
...
def check_beer_by_id(user_id, beer_id):
    with db.cursor() as cursor:
        cursor.execute("SELECT name, category, type, volume, alcohol, country, price, description, photo_url FROM beers WHERE id = %s", (beer_id,))
...
def admins_list(user_id):
    admins = [
        {"name": "@id570718317 | Владислав Миронов", "position": "Разработчик 1"},
        {"name": "@id291170303 | Артём Зимнухов", "position": "Разработчик 2"},
        {"name": "@id242662322 | Сергей Симуков", "position": "Разработчик 3"}
    ]
...
# Функции для отправки кнопок выбора
...
def send_category_buttons(user_id):
    # Функция для отправки кнопок категорий
    send_message(user_id, "Выберите категорию:", buttons=["Светлое", "Тёмное", "Янтарное", "Золотистое"])

def send_subcategory_buttons(user_id):
    # Функция для отправки кнопок подкатегорий
    send_message(user_id, "Выберите подкатегорию:", buttons=["Лагерь", "Эль", "Пшеничное", "Портер"])

# Основной цикл бота
for event in longpoll.listen():
    if event.type == VkEventType.MESSAGE_NEW and event.to_me:
        user_id = event.user_id
        message = event.text.strip() # Приводим текст к нижнему регистру для удобства
        ...
        if message == "начать" or message == "главное меню"or message == "Главное меню"or message == "Начать"or message == "старт"or message == "Старт"or message == "привет"or message == "Привет":
            send_message(
                ...
                buttons=[[{"action": {"label": "Россия"}}, {"action": {"label": "Импортное"}}]]
            )
        ...
        elif user_id in user_states and user_states[user_id].get("stage") == "awaiting_country":
            country = message.capitalize()
            if country in ["Россия", "Импортное"]:
                user_states[user_id] = {"country": country, "stage": "awaiting_category"}
                ...
                send_message(
                    ...
                    buttons=[
                        [{"action": {"label": "Светлое"}}, {"action": {"label": "Тёмное"}}],
                        [{"action": {"label": "Янтарное"}}, {"action": {"label": "Золотистое"}}]
                    ]
                )
            else:
                send_message(user_id, "Пожалуйста, выберите одну из доступных стран: Россия или Импортное.")
            continue

        # Обработка выбора категории
        elif user_id in user_states and user_states[user_id].get("stage") == "awaiting_category":
            ...
            if category in ["Светлое", "Тёмное", "Янтарное", "Золотистое"]:
                ...
                send_message(user_id, f"🎉 Вы выбрали категорию: {category} 🍺\n\n")
                ...
                if subcategories:
                    buttons = [[{"action": {"label": sub[0]}}] for sub in subcategories]
                    send_message(user_id, "Теперь выберите подкатегорию пива 🍻:\n", buttons)
                else:
                    send_message(user_id, "⚠️ Подкатегории для данной категории отсутствуют или данные не найдены 😕.\nПожалуйста, попробуйте выбрать другую категорию 🍻.")
                    user_states[user_id]["stage"] = "awaiting_category"  # Возврат на выбор категории
            else:
                send_message(user_id, "Пожалуйста, выберите одну из доступных категорий: Светлое, Тёмное, Янтарное, Золотистое.")
            continue

        # Обработка выбора подкатегории
        elif user_id in user_states and user_states[user_id].get("stage") == "awaiting_subcategory":
            ...
            if beers:
                ...
                send_message(user_id, f"Пиво в категории '{category}' и подкатегории '{subcategory}' не найдено.")
            continue
